...
##
## 
# Simulate fixed 1:1 Pooled ----------------------------------------------------
message("Simulate Fixed 1:1 Early Stopping Pooled Loss")
simulate_fixed_loss <- function(K, sigma, I_theta_fix, delta, theta_0, theta_tau_0) {
  ...
  for (k in 1:K) {
    ...
    z[k] <- theta_hat[k] * sqrt(Ip_k)
    
    if (z[k] < a_crit[k]) { stop_k <- k; break }
    if (z[k] > b_crit[k]) { stop_k <- k; break }
  }
...
# Run function
run_fixed_prior_bayesloss_chunked <- function(
    cl,
    priors,
    nsims,
    chunk_size = 100000L,
...
res <- run_fixed_prior_bayesloss_chunked(
  cl = cl,
  priors = priors,
  nsims = nsims,
...
print(freqtabfixespl)
# saveRDS(freqtabfixespl, file = file.path("output/expected_losses", "freqtabfixespl.rds"))