...
# Simulate RAR Pooled Loss - Oracle --------------------------------------------
message("Simulate RAR Pooled Loss - Oracle")
simulate_oracle_freqrisk_final_loss <- function(
  ...
  for (k in 1:K) {
    if (k == 1) {
      ...
      n1[k] <- (k / K) * (sigma^2) * I_max * (1 + r)
      n2[k] <- (k / K) * (sigma^2) * I_max * (1 + (1 / r))
    ...
    z <- theta_hat[k] * sqrt(Ip[k])
    if (z < a_crit[k] || z > b_crit[k]) { stop_k <- k; break }
  }
  ...
  for (z in 1:Z) {
    ...
    for (c in 1:n_chunks) {
      ...
      losses_list <- parLapply(
        cl, 1:n_this,
        function(i, z, offset, N, K, sigma, I_theta_fix, delta, oracle, theta_0, theta_tau_0) {
        ...
        z=z, offset=offset,
        N=N, K=K, sigma=sigma, I_theta_fix=I_theta_fix, delta=delta, oracle=oracle,
        theta_0 = theta_0,
        theta_tau_0 = theta_tau_0
      )
...
bayestabraroraclepl <- data.frame(
  prior = 1:nrow(priors),
...
print(bayestabraroraclepl)

#print(res_dir$est); print(res_dir$mcse)
#print(res_abs$est); print(res_abs$mcse)
# saveRDS(bayestabraroraclepl, file = file.path("output/expected_losses", "bayestabraroraclepl.rds"))