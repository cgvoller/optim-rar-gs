...
message("Simulate Whole posterior, accounting for probability of theta + Early Stopping Pooled Loss")
...
## P(S_k | data_{k-1}; theta) via Normal tails on Z_k
...
# Monte Carlo approximation integral at stage k --------------------------------
...
# Golden-section choice of ratio[k] at stage k (k>=2) --------------------------
##
## - posterior at k-1: theta | data_{k-1} ~ N(mu_prev, 1/tau_prev)
## - z_prev, I_prev from stage k-1
## - for each candidate ratio r, set stage-k I1/I2
...
simulate_BRAR_pl_thetapr_final_loss_golden <- function(
    ...
    M_theta = 4000L, r_bounds = c(0.05, 20)
) {
  w_exp <- log(a) / delta
...
bayestabrarprobthetapl <- data.frame(
  prior = 1:nrow(priors),
...
print(bayestabrarprobthetapl)

# saveRDS(bayestabrarprobthetapl, file = file.path("output/expected_losses", "bayestabrarprobthetapl.rds"))
