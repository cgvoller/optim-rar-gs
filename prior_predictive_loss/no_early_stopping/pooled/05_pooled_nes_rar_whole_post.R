...
message("Simulate Bayesian RAR Pooled Loss NS - Whole Posterior")
simulate_BRAR_pl_thetapr_alloc_final_loss <- function(
  ...
  for (k in 1:K) {
    if (k == 1) {
      ...
      n1[k] <- (k / K) * (sigma^2) * I_theta_fix * (1 + r[k])
      n2[k] <- (k / K) * (sigma^2) * I_theta_fix * (1 + (1 / r[k]))
...
print(bayestabrarnswppl)
# saveRDS(bayestabrarnswppl, file = file.path("output/expected_losses", "bayestabrarnswppl.rds"))
