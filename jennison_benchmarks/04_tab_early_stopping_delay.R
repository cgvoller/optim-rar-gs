...
##
## Purpose of script: Table 4: Early stopping with delay
##
## Author: Corey Voller
##
...
## 
# Simulate RAR Pooled ----------------------------------------------------------
message("Simulate RAR Pooled - With early stopping and delay")

simulate_RAR_pl_earlydl <- function(K, mu_1, mu_2, sigma, I_theta_fix,delta) {
  ...
  for (k in 1:K) {
    if (k < 3) {
      ...
      ratio[k] <-  a ** (theta_hat[k - 2] / (2 * delta))
      #ratio[k] <- 1
      # Patients in N1
...
# Initialise arrays
fptb4_results <- initialise_arrays(
  names3D =  c("theta_hats","z","ratio"),
  names2D = c("fptb4_EN1", "fptb4_EN2"),
  nsims = JenNsims,
  theta = theta,
  Ks=rep(K,3)
)
...
for (j in 1:length(theta)) {
  ...
  for (i in 1:JenNsims) {
    fptb4_results$fptb4_EN1[i,j] = unlist(simresults[[i]]$n1[max(which(simresults[[i]]$n1!=0))])
    fptb4_results$fptb4_EN2[i,j] = unlist(simresults[[i]]$n2[max(which(simresults[[i]]$n2!=0))])
    fptb4_results$theta_hats[i,,j] <- unlist(simresults[[i]]$theta_hat)
    fptb4_results$z[i,,j] <- unlist(simresults[[i]]$z)
    fptb4_results$ratio[i,,j] <- unlist(simresults[[i]]$ratio)
  }
}
stopCluster(cl)
end.time <- Sys.time()
...
EN1_pool_se =  apply(fptb4_results$fptb4_EN1, 2, \(x) se_funct2(x))
EN2_pool_se = apply(fptb4_results$fptb4_EN2, 2, \(x) se_funct2(x))
...
message("Simulate RAR Grouped - With early stopping and delay")

simulate_RAR_group_earlydl <- function(K, mu_1, mu_2, sigma, I_theta_fix,delta) {
  # Estimates of mu, theta and ratio
  ...
  for (k in 1:K) {
    if (k < 3) {
      ratio[k] <- 1
      # Initial equal allocation
      n1[k] <- n2[k] <- (1 / K) * 2 * I_max
    } else {
      ratio[k] <-  a ** (theta_hat_group[k - 2] / (2 * delta))
...
# Initialise arrays
fgtb4_results <- initialise_arrays(
  names3D =  c("theta_group","z","ratio"),
  names2D = c("fgtb4_EN1", "fgtb4_EN2"),
  nsims = JenNsims,
  theta = theta,
  Ks=rep(K,3)
)
...
for (j in 1:length(theta)) {
  ...
  update_progress(JenNsims)
  for (i in 1:JenNsims) {
    fgtb4_results$fgtb4_EN1[i,j] = unlist(sum(simresults[[i]]$n1))
    fgtb4_results$fgtb4_EN2[i,j] = unlist(sum(simresults[[i]]$n2))
    fgtb4_results$theta_group[i,,j] = unlist(simresults[[i]]$theta_hat_group)
    fgtb4_results$z[i,,j] = unlist(simresults[[i]]$z)
    fgtb4_results$ratio[i,,j] = unlist(simresults[[i]]$ratio)
  }
}
stopCluster(cl)
end.time <- Sys.time()
...
EN1_group_se =  apply(fgtb4_results$fgtb4_EN1, 2, \(x) se_funct2(x))
EN2_group_se = apply(fgtb4_results$fgtb4_EN2, 2, \(x) se_funct2(x))
...
simulate_NA_earlydl <- function(K, mu_1, mu_2, sigma, I_theta_fix, delta) {
  # Estimates of mu, theta and ratio
  muhat_1 <- muhat_2 <- theta_hat <- ratio <- numeric(K)
  # data from trial
  x_1 <- x_2 <- numeric(K)
...
# Initialise arrays
fttb4_results <- initialise_arrays(
  names2D = c("fttb4_EN1", "fttb4_EN2"),
  nsims = JenNsims,
  theta = theta
)
...
for (j in 1:length(theta)) {
  ...
  update_progress(JenNsims)
  for (i in 1:JenNsims) {
    fttb4_results$fttb4_EN1[i,j] = unlist(simresults[[i]]$n1[max(which(simresults[[i]]$n1!=0))])
    fttb4_results$fttb4_EN2[i,j] = unlist(simresults[[i]]$n2[max(which(simresults[[i]]$n2!=0))])
  }
}
stopCluster(cl)
end.time <- Sys.time()
...
EN1_nadapt_se =  apply(fttb4_results$fttb4_EN1, 2, \(x) se_funct2(x))
EN2_nadapt_se = apply(fttb4_results$fttb4_EN2, 2, \(x) se_funct2(x))
...
# Table
freq_tb4 <- create_tab_fun(EN1_nadapt,
                           EN2_nadapt,
                           total_nadapt,
                           EN1_pool,
                           EN2_pool,
                           ...
                           EN2_group,
                           total_group,
                           headertxt = "Designs with early stopping and one group delay",
