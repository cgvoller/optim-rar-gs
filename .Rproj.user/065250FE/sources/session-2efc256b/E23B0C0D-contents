...
##
## Purpose of script: Losses using whole posterior with early stopping
##
## Author: Corey Voller
##
...
## 
# Simulate RAR Pooled ----------------------------------------------------------
message("Simulate Bayesian RAR Pooled Loss - Whole Posterior")
simulate_BRAR_pl_thetapr_alloc_final_loss <- function(
    N, K, theta, sigma, I_theta_fix, delta, theta_0, theta_tau_0
) {
  ...
  for (k in 1:K) {
    if (k == 1) {
      ...
      n1[k] <- (k / K) * (sigma^2) * I_max * (1 + r[k])
      n2[k] <- (k / K) * (sigma^2) * I_max * (1 + (1 / r[k]))
    ...
    for (c in 1:n_chunks) {
      ...
      losses_list <- parLapply(
        ...
        function(i, z, offset, N, K, sigma, I_theta_fix, delta, theta_0, theta_tau_0) {
          ...
          simulate_BRAR_pl_thetapr_alloc_final_loss(
            N = N, K = K,
            sigma = sigma,
...
print(bayestabrarwppl)
#saveRDS(bayestabrarwppl, file = file.path("output/expected_losses", "bayestabrarwppl.rds"))
