...
message("Simulate Bayesian RAR Grouped Loss NS - Whole Posterior")
simulate_BRAR_grp_thetapr_alloc_final_loss <- function(
  ...
  for (k in 1:K) {
    if (k == 1) {
      ...
      n1[k] <- (1 / K) * (sigma^2) * I_theta_fix * (1 + r[k])
      n2[k] <- (1 / K) * (sigma^2) * I_theta_fix * (1 + (1 / r[k]))
...
print(bayestabrarnswpgrp)
# saveRDS(bayestabrarnswpgrp, file = file.path("output/expected_losses", "bayestabrarnswpgrp.rds"))
