...
##
## Purpose of script: Frequentist Fixed 1:1 No early stopping
##
## Author: Corey Voller
##
...
##
## 
# Simulate fixed 1:1 NES Pooled ------------------------------------------------
message("Simulate Fixed 1:1 No Early Stopping Pooled Loss")
simulate_fixed11_prior_bayesloss_final <- function(
  ...
  theta_hat <- rep(NA_real_, K)
  
  # data from trial (stage means)
  x_1 <- x_2 <- rep(NA_real_, K)
  ...
  # posterior objects (normal-normal on theta)
  ...
  for (k in 1:K) {
    ...
    # Sample once from normal distribution (stage means)
    ...
    for (c in 1:n_chunks) {
      ...
      losses_list <- parLapply(
        ...
        function(i, z, offset, K, sigma, I_theta_fix, delta, theta_0, theta_tau_0) {
          ...
          simulate_fixed11_prior_bayesloss_final(
            K = K,
            sigma = sigma,
...
res <- run_fixed11_prior_bayesloss_chunked(
  cl = cl,
  priors = priors,
  nsims = nsims,
...
print(freqtabfixnspl)
#saveRDS(freqtabfixnspl, file = file.path("output/expected_losses", "freqtabfixnspl.rds"))
