...
##
## 
# Simulate BRAR Grouped No early stopping ---------------------------------------
message("Simulate Bayesian RAR Grouped Loss NS - Posterior Mean")
simulate_BRAR_grp_thetapr_final_loss <- function(
  ...
  for (k in 1:K) {
    ...
    mu_post[k]  <- (theta_tau_0 * theta_0 + Ip_k * theta_hat_group[k]) / tau_post[k]
    ...
    for (c in 1:n_chunks) {
      ...
      losses_list <- parLapply(
        ...
        function(i, z, offset, K, sigma, I_theta_fix, delta, theta_0, theta_tau_0) {
          ...
          simulate_BRAR_grp_thetapr_final_loss(
            K = K,
            sigma = sigma,
...
print(bayestabrarnsgrp)
# saveRDS(bayestabrarnsgrp, file = file.path("output/expected_losses", "bayestabrarnsgrp.rds"))
