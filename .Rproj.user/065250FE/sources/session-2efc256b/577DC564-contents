...
##
## Purpose of script: Table 2: No early stopping (freq pooled+group)
##
## Author: Corey Voller
##
...
## 
# Simulate RAR Pooled ----------------------------------------------------------
message("Simulate RAR Pooled - No early stopping w delay")
simulate_RAR_delay <- function(N, K, mu_1, mu_2, sigma, I_theta_fix,delta) {
  ...
  for (k in 1:K) {
    if (k < 3) {
      ...
      n1.new[k] <- n2.new[k] <- (1 / K) * 2 * I_theta_fix
      n1[k] <- n2[k] <- sum(n1.new[1:k])
    } else {
...
# Initialise arrays
fptb2_results <- initialise_arrays(
  names3D =  c("theta_hats"),
  names2D = c("fptb2_EN1", "fptb2_EN2"),
  nsims = JenNsims,
  theta = theta,
  Ks=K
)
...
for (j in 1:length(theta)) {
  ...
  for (i in 1:JenNsims) {
    fptb2_results$fptb2_EN1[i, j] = unlist(simresults[[i]]$n1[K])
    fptb2_results$fptb2_EN2[i, j] = unlist(simresults[[i]]$n2[K])
    fptb2_results$theta_hats[i,,j] <- unlist(simresults[[i]]$theta_hat)
  }
}
stopCluster(cl)
end.time <- Sys.time()
...
EN1_pool_se =  apply(fptb2_results$fptb2_EN1, 2, \(x) se_funct2(x))
EN2_pool_se = apply(fptb2_results$fptb2_EN2, 2, \(x) se_funct2(x))
...
message("Simulate RAR Grouped - No early stopping w delay")

simulate_RAR_group_dl <- function(K, mu_1, mu_2, sigma, I_theta_fix,delta) {
  ...
  for (k in 1:K) {
    if (k < 3) {
      ratio[k] <- 1
      # Initial equal allocation
      n1[k] <- n2[k] <- (1 / K) * (sigma ^ 2) * I_theta_fix * (1 + ratio[k])
    } else {
...
# Initialise arrays
fgtb2_results <- initialise_arrays(
  names3D =  c("theta_group"),
  names2D = c("fgtb2_EN1", "fgtb2_EN2"),
  nsims = JenNsims,
  theta = theta,
  Ks=K
)
...
for (j in 1:length(theta)) {
  ...
  update_progress(JenNsims)
  for (i in 1:JenNsims) {
    fgtb2_results$fgtb2_EN1[i, j] = unlist(sum(simresults[[i]]$n1))
    fgtb2_results$fgtb2_EN2[i, j] = unlist(sum(simresults[[i]]$n2))
    fgtb2_results$theta_group[i, , j] = unlist(simresults[[i]]$theta_hat_group)
  }
}
stopCluster(cl)
end.time <- Sys.time()
...
EN1_group_se =  apply(fgtb2_results$fgtb2_EN1, 2, \(x) se_funct2(x))
EN2_group_se = apply(fgtb2_results$fgtb2_EN2, 2, \(x) se_funct2(x))
...
# Table
freq_tb2 <- create_tab_fun(EN1_target,
                           EN2_target,
                           target_total,
                           EN1_pool,
                           EN2_pool,
                           ...
                           EN2_group,
                           total_group,
                           headertxt = "Designs with no early stopping and one group delay",
