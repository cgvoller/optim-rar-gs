...
##
## Purpose of script: Frequentist RAR MLE with early stopping
##
## Author: Corey Voller
##
...
message("Simulate RAR With Early Stopping Group Loss")
simulate_RAR_group_loss <- function(K,
                                    mu_1,
                                    mu_2,
                                    sigma,
                                    I_theta_fix,
  ...
  c(
    sum_post_loss = sum(loss_stage_post),
    sum_realised_loss = sum(loss_stage_real)
  )
}


# Run function
run_raresgrp_prior_bayesloss_chunked <- function(cl,
                                                 priors,
                                                 nsims,
                                                 chunk_size = 100000L,
                                                 K = 5L,
  ...
  for (z in 1:Z) {
    ...
    for (c in 1:n_chunks) {
      ...
      post <- losses_mat[, "sum_post_loss"]
      real <- losses_mat[, "sum_realised_loss"]
...
resgrp <- run_raresgrp_prior_bayesloss_chunked(
  cl = cl,
  priors = priors,
  nsims = nsims,
...
freqtabraresgrp <- data.frame(
  prior = 1:nrow(priors),
  theta_0 = priors$theta_0,
  theta_tau0 = priors$theta_tau0,
  est_post = resgrp$est_post,
  mcse_post = resgrp$mcse_post,
  est_real = resgrp$est_real,
  mcse_real = resgrp$mcse_real
)
print(freqtabraresgrp)
# saveRDS(freqtabraresgrp, file = file.path("output/expected_losses", "freqtabraresgrp.rds"))
