...
## 
# Oracle functions -------------------------------------------------------------
oracle_ratio_abs <- function(theta, a, delta)
  a ^ (abs(theta) / (2 * delta))
oracle_ratio_dir <- function(theta, a, delta)
  a ^ (theta / (2 * delta))  # <1 if theta<0

# Simulate RAR Group Loss - Oracle NES ----------------------------------------\
message("Simulate RAR Group Loss - Oracle NES")
simulate_oracle_freqrisk_final_loss <- function(K,
  ...
  for (k in 1:K) {
    if (k == 1) {
      ...
      n1[k] <- (1 / K) * (sigma ^ 2) * I_theta_fix * (1 + r)
      n2[k] <- (1 / K) * (sigma ^ 2) * I_theta_fix * (1 + (1 / r))
    }
    
    mu1[k] <- rnorm(1, mean = theta_true, sd = sqrt(sigma ^ 2 / n1[k]))
    mu2[k] <- rnorm(1, mean = mu_2, sd = sqrt(sigma ^ 2 / n2[k]))
...
bayestabraroraclensgrp <- data.frame(
  prior = 1:nrow(priors),
...
print(bayestabraroraclensgrp)

#print(res_dir$est); print(res_dir$mcse)
#print(res_abs$est); print(res_abs$mcse)
# saveRDS(bayestabraroraclensgrp, file = file.path("output/expected_losses", "bayestabraroraclensgrp.rds"))
